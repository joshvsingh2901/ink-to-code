...
- production-grade sandboxing (Docker locally, Modal for hosted production) is required and already implemented — see "Isolated C++ execution" below; do not weaken it or bypass it.
...
- Student code never executes as a bare host subprocess; it always runs through the isolated execution provider (Docker locally, Modal for hosted production) described in "Isolated C++ execution" below.
- That execution provider is the production-grade sandboxing this project requires before exposing arbitrary code execution to real users; do not bypass it.
...
- User-submitted C++ must never be compiled or executed directly on the API host. All compile and run paths — normal Compile, manual tests, AI-generated tests, Rerun Same Tests, object scenarios, and memory diagnostics — go through the configured execution provider (`execution_providers.py`'s `ExecutionProvider` interface), currently either Docker (local, the default) or Modal (isolated cloud sandboxes). No other module may invoke a compiler or a compiled binary directly; this is enforced by a static guard test, not just convention.
- Execution must fail closed: if the configured provider's runtime or runner image is unavailable, the request reports an infrastructure error. There is no host fallback, ever, for either provider.
- Do not reintroduce host `g++`/`clang++`/compiled-binary execution outside the execution-provider boundary.
- Submitted code must run as a non-root user with no network and strict CPU, memory, process, filesystem, and timeout limits, on both providers.
- Never mount the repository, secrets, home directory, or the Docker socket into the Docker provider; never pass a Modal secret, volume, or network filesystem into a sandbox — every execution environment on both providers is hermetic (no host or platform credential is ever visible to user code).
- Never use privileged Docker containers. On Modal, never set `secrets`, `outbound_cidr_allowlist`, `outbound_domain_allowlist`, `inbound_cidr_allowlist`, `encrypted_ports`, `unencrypted_ports`, `h2_ports`, `proxy`, `custom_domain`, or any port/tunnel option on a sandbox; build its image only via `Image.from_registry` (no `run_commands`/`pip_install`/`apt_install` per request); create at most one sandbox per provider instance (one per compile-and-test request), reused across that request's compile and all of its test runs, never pooled or reused across HTTP requests; and never read the sandbox's result file through the sandbox filesystem API directly — always through the dedicated root-side reader (`read_result.py`), which rejects a symlink at the expected result path instead of following it, mirroring the Docker provider's own `_is_regular_result_file` guard.
- Frontend requests cannot control provider commands, images, compiler flags, mounts/volumes, environment variables, or resource limits, on either provider.
- LeakSanitizer and Valgrind capability must be proven with deliberate-leak probes, per provider.
- Infrastructure failures are distinct from student-code failures.
- All containers/sandboxes and temporary files must be removed after execution (Docker: `--rm`; Modal: `sandbox.terminate()` in a `finally`, backstopped by its configured idle timeout).
- Diagnostics must redact host/container paths and remain size-limited.
- Docker and Modal are not identical in every control — see the security matrix in `docs/internal/MODAL_DEPLOYMENT.md` and do not overclaim: Modal's sandbox container runs as root and ignores a Dockerfile `USER` directive, so non-root execution on that path is a compensating control (`setpriv` drop-privilege inside the fixed launch command), not a container-native guarantee; Modal has no read-only-rootfs equivalent at all (compensated only by every sandbox being single-use and ephemeral); Modal's CPU limit is a soft throttle rather than Docker's hard CFS quota (the wall-clock timeout remains the real control on both providers). Never describe the Modal path as providing read-only rootfs, capability dropping, or a proven PID limit without a live test backing that specific claim.
