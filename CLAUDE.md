...
Stack: Next.js/TypeScript/Tailwind/App Router frontend (`frontend/`), FastAPI/Pydantic backend (`backend/`), optional Docker-based isolated C++ execution runner (`runner/`).
...
Set `CPP_EXECUTION_PROVIDER=auto` in `backend/.env` to prefer this runner for memory diagnostics when Docker is available; falls back to the host provider otherwise (never silently falls back in `docker` mode — reports infrastructure-unavailable instead).
...
- **`app/services/execution_providers.py`** — abstracts "host subprocess" vs. "isolated Docker runner" execution so sanitizer/Valgrind-based memory diagnostics can run in a locked-down container (no network, non-root, fixed resource limits) when available, and cleanly reports infrastructure-unavailable otherwise.
